  ...
  function handlePrint() {
    ...
    for (const m of months) {
      ...
      for (const day of days) {
        ...
        weekdayRow += `<td style="${cellStyle}background:${wdBg};color:${wdColor};font-size:9px;font-weight:600;">${WEEKDAY_LABELS[dow]}</td>`;
        dateRow += `<td style="${cellStyle}background:${isOff ? "#f1f5f9" : "#fff"};color:${isOff ? "#94a3b8" : "#334155"};font-size:10px;font-weight:700;">${day}</td>`;
      }
      ...
      for (const day of days) {
        ...
        const textStyle = text ? "writing-mode:vertical-rl;font-size:8px;line-height:1.2;color:#1e293b;font-weight:500;" : "";
        bruRow += `<td style="${cellStyle}background:${cellBg};vertical-align:top;"><span style="${textStyle}">${text}</span></td>`;
      }
      ...
      for (const day of days) {
        ...
        const cellBg = isOff ? "#f1f5f9" : "#fff";
        const textStyle = text ? "writing-mode:vertical-rl;font-size:8px;line-height:1.2;color:#1e293b;font-weight:500;" : "";
        clientRow += `<td style="${cellStyle}background:${cellBg};vertical-align:top;"><span style="${textStyle}">${text}</span></td>`;
      }

      tablesHtml += `
        <table style="table-layout:fixed;border-collapse:collapse;margin-bottom:24px;width:100%;">
          <thead>
            <tr>
